...
`commentCount`는 목록 HTML에 이미 있는 값만 가져오므로 상세 페이지 요청을 늘리지 않는다. 값이 없거나 확인할 수 없으면 `null`로 두며, 댓글 수는 반응량일 뿐 긍정 평가가 아니다. 현재는 출처 게시글별 Deal 카드에 중립적으로 표시하고 추천·정렬에 사용하지 않는다. 향후 여러 출처를 DealGroup으로 묶더라도 단순 합산하지 않고 출처별로 보여주는 방향만 유지한다.
