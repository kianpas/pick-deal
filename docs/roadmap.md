...
> 최초 작성: 2026-08-20 · 최근 갱신: 2026-08-26 (작업 1~9 완료 반영 + 검증 게이트 추가)  
...
- 이후 가격 이력, 댓글 반응, 알림, AI 구매 판단 보조로 확장한다.
...
현재 재수집 시 가격이 바뀌면 최신 가격으로 갱신한다.

가격 변경 직전에 이력을 남기면 큰 비용 없이 구매 판단 데이터를 만들 수 있다.
...
또는 DealGroup 기준으로 가격을 추적할 필요가 생기면 이후 재검토한다.
...
| 10. 가격 이력 | Phase 6 | 미착수 |
| 11. OCI 상시 배포 | Phase 7 | 미착수 |
| 12. 알림 | Phase 8 | 미착수 |
| 13. AI 댓글 요약 | Phase 9 | 미착수 |

**다음 우선 후보**: 작업 7의 후속(상품 URL 확보율 올리기). 이게 오르기 전에는 작업 8·9가 화면에 아무 효과도 내지 못한다. 리뷰 상세는 `docs/notes/2026-08-26-work-review-and-fixes.md`.
...
7-1. 상품 URL 확보율 올리기
     - 상세 보강이 신규 딜에만 적용되고 max-detail-requests(3)로 제한된다
     - 과거 딜 백필 여부는 출처 부담과 맞바꾸는 판단 → 별도 결정
     - 이게 올라가야 8·9가 화면에 효과를 낸다
