...
- **현재 구현**: 코드 기반 `config/SeedDataInitializer`(기동 시 더미 출처/딜 삽입). H2 `create-drop`이라 매 기동 시 재적재된다.
...
확장 단계에 가면 `user`(멀티유저), `deal_comment`/`deal_comment_summary`(AI 요약), `keyword_alert`/`notification`(알림), `deal_group` 또는 `deal.group_id`(중복 그룹) 등이 추가될 수 있다. **구체 스키마는 각 기능에 착수할 때 설계한다**(지금 미리 그리지 않는다).
