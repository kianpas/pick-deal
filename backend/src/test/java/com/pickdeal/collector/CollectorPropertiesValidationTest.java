...
class CollectorPropertiesValidationTest {
    ...
    void rejectsNonPositiveLimits() {
        QuasarzoneCollectorProperties properties =
                new QuasarzoneCollectorProperties(true, Duration.ofSeconds(10), 0, 0);

        assertThat(validator.validate(properties))
                .extracting(violation -> violation.getPropertyPath().toString())
                .containsExactlyInAnyOrder("maxPages", "maxItems");
    }
    ...
    void rejectsInvalidTimeout() {
        RuliwebCollectorProperties zero =
                new RuliwebCollectorProperties(true, Duration.ZERO, 1, 1);
        RuliwebCollectorProperties tooLarge =
                new RuliwebCollectorProperties(true, Duration.ofMillis((long) Integer.MAX_VALUE + 1), 1, 1);
