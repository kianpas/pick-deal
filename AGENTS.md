...
- **다음에 뭘 할지는 `docs/roadmap.md`가 기준이다.** 현재 진행 상태(작업 ↔ Phase 매핑), 실측 수치, 작업 순서, 그리고 **모든 작업의 공통 완료 조건**(§5.1 — fixture 테스트만으로 끝내지 말고 실제 1회 수집해 확보율을 확인한다)이 거기 있다. 작업을 완료하면 로드맵의 현재 상태와 작업 목록도 함께 갱신한다.
