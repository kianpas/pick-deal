...
수집기는 `collector/` 패키지에 있다(2차 진입). 출처별 하위 패키지(`collector/quasarzone/`)에 fetch(Client)·parse(Parser)·normalize/persist(CollectService)를 두고, `CollectScheduler`(`@Scheduled`)가 주기 실행한다. 새 출처 추가 = 새 하위 패키지. 파서는 `String html → 결과` 순수 함수로 두고 실제 응답 HTML 픽스처(`src/test/resources/fixtures/`)로 테스트한다.
